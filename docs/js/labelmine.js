...
function canvasToBlob(canvas, type = 'image/png', quality) {
  ...
  SDF.Icons = (function(){
    ...
    function check(size=18, { strokeWidth=2.1 }={}){
    ...
    function reset(size=18, { strokeWidth=2.1 }={}){
      const svg = mk(size); const p=document.createElementNS(ns,"path");
    ...
    function imp(size=18, { strokeWidth=1.75 }={}){
      const svg = mk(size); const p=document.createElementNS(ns,"path");
...
(function(){
  if (!btnSave) return;
  const form = btnSave.closest ? btnSave.closest("form") : null;
  if (form && !form.__saveSubmitGuard__) {
    const onSubmit = (e)=>{
      const s = e.submitter || null;
      if (s && s === btnSave) { e.preventDefault(); e.stopPropagation(); }
    };
    form.addEventListener("submit", onSubmit, true);
    form.__saveSubmitGuard__ = onSubmit;
  }
})();

// Guard Save button: avoid implicit form submit and bubbling
if (btnSave) {
  try { if (!btnSave.type) btnSave.type = "button"; } catch {}
  if (!btnSave.__pm_pd_guard__) {
    const __pd = (ev)=>{ ev.stopPropagation(); };
    btnSave.addEventListener("pointerdown", __pd, { passive: true });
    btnSave.__pm_pd_guard__ = __pd;
  }
}
...
(function GalleryHorizontalModule() {
  ...
  ensureReady(() => {
    ...
    const SVG_TRASH = `
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" class="gh-icon gh-icon--trash">
      <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
    </svg>
    `.trim();

    const SVG_DOWNLOAD = `
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" class="gh-icon gh-icon--download">
